...
// @ts-ignore
import gCode from './fruchterman.g';

// @see https://github.com/facebook/create-react-app/issues/8014
/* eslint-disable */
const ctx: Worker = self as any;

const MAX_ITERATION = 8000;
let maxEdgePerVetex = 0;
const edgeIndexBufferData: number[] = [];
...
function buildTextureData(
  ...
  const nodeDict: Array<Array<number>> = [];
  const mapIdPos: {
    [id: string]: number;
  } = {};
  let i = 0;
...
ctx.onmessage = async (evt) => {
  ...
  const nodes = data.nodes.map((n: { id: number }) => ({
  ...
  // 使用从主线程传递来的 OffscreenCanvas
  const canvas = evt.data.canvas;

  const world = new World({
    canvas, // 传入 OffscreenCanvas
    engineOptions: {
      supportCompute: true,
    },
  });
